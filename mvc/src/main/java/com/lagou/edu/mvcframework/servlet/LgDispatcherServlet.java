...
//        method.invoke() //
    ...
    private boolean hasPermission(Handler handler, HttpServletRequest req) {
        LagouSecurity security = handler.getMethod().getAnnotation(LagouSecurity.class);
        if (security == null) {
            security = handler.getMethod().getDeclaringClass().getAnnotation(LagouSecurity.class);
        }
        // 如果标记了注解，则判断权限
        String[] value = security.value();
        if (security != null && value.length > 0) {
            String username = req.getParameter("username");
            for (String per : value) {
                if (per.equals(username)) {
                    return true;
                }
            }
            return false;
